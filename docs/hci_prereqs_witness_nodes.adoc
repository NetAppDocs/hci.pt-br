...
Se você estiver realizando uma atualização in-place de um Witness Node existente para o Element 12.8 ou posterior, antes de atualizar, certifique-se de que a versão do hardware da VM no Witness Node seja compatível com o ESXi 6.7 (versão de hardware da VM 14) ou posterior, dependendo do seu ambiente. Verlink:task_hcc_upgrade_management_node.html#upgrade-vm-hardware["Atualize a versão do hardware da VM"] .
